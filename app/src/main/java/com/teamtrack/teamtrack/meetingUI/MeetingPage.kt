...
fun MeetingPage(onMeetingCreated: (Meeting) -> Unit) {
    ...
    Column(
        ...
        // 참석자 선택을 위한 드롭다운 메뉴
        DropdownAttendees(selectedAttendees, onClickAttendees = { attendee ->
            // 클릭 시 선택/선택 해제 토글
...
@Composable
fun DropdownAttendees(selectedAttendees: Set<String>, onClickAttendees: (String) -> Unit) {
    // 참석 가능한 인원 목록
        ...
        DropdownMenu(
            ...
            attendees.forEach { attendee ->
                DropdownMenuItem(
                    ...
                    onClick = { onClickAttendees(attendee) }
                )
            }
        }
    }
